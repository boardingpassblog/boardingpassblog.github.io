    ...
    function displaySearchDirectorio(results, store) {
      ...
      if (results.length) { // Are there any results?
        ...
        for (var i = 0; i < results.length; i++) {  // Iterate over the results
          ...
          iterador += 1;
           // Crear una nueva fila para cada par de tarjetas
          if (i % 2 === 0) {
          ...
          if (item.featured === "true"){
            ...
            appendString +='<div><i class="fas fa-building"></i> '+item.name+'</div>';
            appendString +='<div><i class="fas fa-map-marker-alt"></i> '+item.address+'</div>';
            appendString +='<div><i class="fas fa-bullhorn"></i> '+item.services+'</div>';
            appendString +='<div><i class="fas fa-industry"></i> '+item.sector+'</div>';
            appendString +='</div>';
            appendString +='</div>';
            appendString +='</div>';
            ...
            appendString +='<a href="tel:'+item.phone_number+'"> '+item.phone_number+'</a>';
            appendString +='</div>';
            appendString +='<div>';
          ...
          // Cerrar la fila después de generar dos tarjetas
